...
public class ImaPlayer {
    ...
    public void onStateChanged(boolean playWhenReady, int playbackState) {
      if (playbackState == ExoPlayer.STATE_ENDED) {
        ...
        for (VideoAdPlayer.VideoAdPlayerCallback callback : callbacks) {
          callback.onEnded();
          System.out.println("INSIDE ExoPlayer.STATE_ENDEDExoPlayer.STATE_ENDEDExoPlayer.STATE_ENDED");
        }
      }
  ...
  public ImaPlayer(Fragment activity,
    ...
    this.activity = activity;
      this.playbackEnded = (PlaybackEnded) activity;
   ...
   */
  public void play() {
    if (adTagUrl != null && adTagUrl.toString().length() > 0) {
      requestAd();
    } else {
      contentPlayer.play();
    }
